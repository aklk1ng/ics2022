...
#define immA() do { *imm = SEXT(BITS(i, 31, 25), 7); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , A, R(rd) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , A, R(rd) = src1 - src2);
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , A, R(rd) = src1 < src2);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , A, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , A, R(rd) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , A, R(rd) = src1 & src2);
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , S, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , S, R(rd) = (SEXT(src1, 64) * SEXT(src2, 64)) >> 32);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , S, R(rd) = src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , S, R(rd) = src1 % src2);
