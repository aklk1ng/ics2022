...
uint32_t calculate(int p, int q) {
  if (p >= q) {
    printf("wrong");
    return 0;
  }
  int count = p, num_of_stack = 0, sign[100] = {0}, tmp = 0, i = 0;
  uint32_t temp1 = 0, temp2 = 0, temp = 0, stack[100] = {0}, val1 = 0,
               val2 = 0;
  bool flag = true;
  ...
  while (count < q) {
    ...
    case ')':
      ...
      while (sign[num_of_stack] != '(') {
        ...
        case '-':
          if (stack[num_of_stack - 2] != '(' &&
              sign[num_of_stack - 2] == sign[num_of_stack]) {
...
word_t expr(char *e, bool *success) {
  ...
  for (int i = 0; i < nr_token; i++) {
    ...
    uint32_t res = calculate(0, nr_token);
    *success = true;
    return res;
  }
